...
im_process <- function(label, load.path, out.path, d.thr, flood, fill, obj.thr, stained.thr){

    im <- load.image(paste0(load.path, label, ".jpg"))
    save.image(im, paste0(out.path, label, ".jpg"))
    ...
    # Alizarin Red: >0.45; Von Kossa: > 0.65
    stained <- as.cimg(ifelse(g2d1> stained.thr & ROI2d!=0, 1, 0))
...
### load filenames
in.Path <- "I:/Study/projects/Pellet Data/Paper/Figure/F3/AR50in/"
out.Path <- "I:/Study/projects/Pellet Data/Paper/Figure/F3/AR50out/"

label_list <- list.files(path = in.Path,
                         pattern = "*.jpg",
                         full.names=FALSE)
filenames <- substr(label_list, 1, nchar(label_list)-4)

### execute image process function
out_stats <- NULL
for (i in 1:length(filenames)){
  im_label <- filenames[i]

  stats <- im_process(label = im_label,
                      load.path = in.Path,
                      out.path = out.Path,
  ...
  stats <- cbind(im_label, stats)

# stack results
out_stats <- rbind(out_stats, stats)
}

write.csv(out_stats, paste0(out.Path, "out_stats.csv"))
